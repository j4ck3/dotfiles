...
EOF
        ...
        if [[ "$http_code" -ge 200 && "$http_code" -lt 300 ]]; then
            ...
            if echo "$config" | grep -q "$HOMESERVER_DEVICE_ID"; then
                log_info "Homeserver device is already configured - enabling auto-accept..."
...
" 2>/dev/null | head -1 | tr -d '\n\r') || true
    ...
    while [[ $attempts -lt $max_attempts ]]; do
        ...
        # Check if status response indicates an actual error (not the "errors" count field)
        if [[ -n "$status" ]]; then
            # Check for actual error object, not the "errors" count field
            if echo "$status" | python3 -c "import sys, json; s=json.load(sys.stdin); sys.exit(0 if s.get('error') else 1)" 2>/dev/null; then
                local error_msg
    ...
    if [[ -f "$SYNC_HOST_PATH/uuid-mapping.json" ]]; then
        ...
        echo "   - Check if it shows 'Up to date' or 'Syncing'"
        echo "   - Look for any error messages"
        echo ""
        echo "3. Check Syncthing UI on homeserver (tower): http://10.0.0.24:8384"
        echo "   - Verify 'zen-private' folder is shared with this device"
        echo "   - Check folder sync status"
        echo ""
        echo "4. Verify both devices are connected:"
        echo "   - In Syncthing UI, check 'Remote Devices'"
        echo "   - Make sure 'tower' device shows as 'Connected' (green indicator)"
        echo ""
        echo "5. Check folder configuration:"
        echo "   - On this machine: Ensure folder path is /syncthing/zen-private (in container)"
        echo "   - On homeserver: Ensure folder is shared with this device's ID"
        echo "   - Check if folder type is 'Receive Only' on this machine"
        echo ""
        echo "6. Check network connectivity:"
        echo "   - If using Tailscale: tailscale ping tower"
        echo "   - If local network: ping 10.0.0.24"
        echo ""
        echo "7. If sync is stuck, try:"
        echo "   - Restart Syncthing: docker restart syncthing"
        echo "   - Check logs: docker logs syncthing"
        echo "   - In Syncthing UI: Click 'Rescan' on the folder"
        echo ""
